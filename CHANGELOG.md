...
- `graylog_index_set`: creation now waits for Graylog's own index-set initializer to make the deflector healthy and point it to a concrete write index before returning. The provider does not issue manual cycle requests, avoiding races and duplicate allocation work with Graylog's background initializer. Acceptance coverage creates eight index-set/stream pairs concurrently, verifies every live deflector, and confirms that every live stream points to its paired index set.
...
- **`graylog_cluster_config`**: new resource wrapping `/system/cluster_config/{class}`, Graylog's store of cluster-wide settings keyed by the fully qualified name of the class that reads them. Many of those settings have no dedicated API, so they previously had to be driven by out-of-band `curl` calls from a `null_resource` provisioner; a single generic resource covers all of them. `class` forces replacement (a different class is a different document) and `Delete` removes the stored document, which returns the class to the default compiled into the server.
  ...
  Whole-document semantics, verified live: Graylog deserializes the body into the target class and **rejects incomplete documents** (`Couldn't parse cluster configuration ... Null <field>`), so this is a replace and never a patch. For the classes probed, the server stores what it is given verbatim; state keeps the practitioner's document rather than the server echo — the failure mode that produced the `graylog_input`/extractors "Update ran on every single apply" bug in v0.5.0. `Read` compares canonical JSON and only adopts the server document when it actually differs, so formatting and key order never cause a diff. An integration test asserts that verbatim round-trip for the probe classes on every supported major version. Classes with encrypted fields, such as `GeoIpResolverConfig`, are not verbatim — see the Fixed entry below.
...
### Fixed
- **`graylog_cluster_config`: perpetual diff on classes that do not store the document verbatim.** `Read` compared the whole server echo against `config_json`, which converges only when Graylog stores exactly what it was given. `org.graylog.plugins.map.config.GeoIpResolverConfig` on 7.1.5 does not: a seven-key document reads back with eight more (`azure_cloud`, `use_s3`, `pull_from_cloud`, `gcs_project_id`, `azure_account`, `azure_container`, `azure_endpoint`, and the `EncryptedValue` sentinel `azure_account_key: {"is_set": false}`), so every plan proposed removing them. The echo cannot even be written back — a `PUT` of it verbatim answers `400 Couldn't deserialize value: {"is_set":false} (set_value must be a string and cannot be missing)` — so no writable document could ever equal what `Read` compared against. `Read` now projects the server document onto the keys present in state before comparing, the same approach `graylog_lookup_cache`/`graylog_lookup_adapter` already use: server-materialized defaults are ignored, a managed key changed server-side is still reported. Import, which has no document to project against, still adopts the whole server document, minus `{"is_set": ...}` sentinels, so what lands in state is something that can be applied. Verified live on 7.1.5: empty plan with the practitioner's document, an API-side `enabled: false` still detected, clean destroy. The "stores verbatim" statement in the `graylog_cluster_config` entry above holds for the classes the integration test probes, not for every class.
...
- **`graylog_user`**: an unset `session_timeout_ms` was sent to Graylog as `0`. The create is accepted and basic-auth API access keeps working, but the user's first interactive login fails with HTTP 500 `Session timeout is set to 0 seconds`, which is why it went unnoticed until a pre-created LDAP user tried to sign in. The `User` struct already had `omitempty`; the Graylog 5/6/7 create path builds its own request map and bypassed it. The key is now omitted when unset so Graylog applies its default (8 hours); the attribute is `Optional`+`Computed` and reads the server's value back into state (this also removes the `+ session_timeout_ms` diff after `terraform import`), and `0` is rejected at plan time. Users created by earlier versions without the attribute still have `0` stored server-side: set `session_timeout_ms` explicitly once to repair them. Reproduced live on Graylog 7.1.5.
- **`graylog_user`**: `Update` re-sent the password from state on every apply, so any unrelated change (e.g. `roles`) on an external (LDAP/AD) user failed with HTTP 403 `Cannot change password for external user`. The password change is now its own client call (`SetUserPassword`) that `Update` makes only when the planned password differs from the one in state; `UpdateUser` no longer touches the password endpoint. `password` is `Optional`+`Computed` with a plan modifier that keeps the state value when the attribute is not configured, so removing it after the first login is not a change and re-adding the same value is a no-op. Create still requires a password (Graylog answers 400 `Null password` without one). Two server rules surfaced by the same test are now documented on the resource: `full_name` needs at least two words on create (500 `A lastName value is required`), and for external users the directory overwrites `full_name`/`email` on every login while roles stay Terraform-owned (Graylog does not merge the backend's `default_roles` into a pre-existing profile). Reproduced live on Graylog 7.1.5.
- **`graylog_user`**: `disabled` was `Optional` without `Computed` while `Read` unconditionally stored the server's `false`, so every configuration that omitted the attribute got a perpetual `- disabled = false -> null` plan (exit code 2 on every `plan -detailed-exitcode`). Same class as the `session_timeout_ms` bug above and fixed the same way: `Optional`+`Computed` with `UseStateForUnknown`, filled from the server response when the plan leaves it unknown. Omitting it now means "leave the account as it is"; an explicit `disabled = false` against an existing state keeps planning clean. `TestAccUser_basic` no longer needs `ExpectNonEmptyPlan`. Reproduced live on Graylog 7.1.5 and 6.0.14.
